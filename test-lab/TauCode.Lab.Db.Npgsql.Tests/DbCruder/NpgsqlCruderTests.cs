...
namespace TauCode.Lab.Db.Npgsql.Tests.DbCruder
{
    [TestFixture]
...
CREATE TABLE ""zeta"".""MediumTable""(
    ...
    ""TheNVarChar"" character varying(100) NULL DEFAULT 'Semmi')
";
...
";
        ...
        public void Constructor_ValidArguments_RunsOk(string schemaName)
        {
            ...
            // Act
            IDbCruder cruder = new NpgsqlCruderLab(this.Connection, schemaName);

            // Assert
            Assert.That(cruder.Connection, Is.SameAs(this.Connection));
            Assert.That(cruder.Factory, Is.SameAs(NpgsqlUtilityFactoryLab.Instance));
            Assert.That(cruder.SchemaName, Is.EqualTo("public"));
            Assert.That(cruder.ScriptBuilder, Is.TypeOf<NpgsqlScriptBuilderLab>());
        ...
        public void Constructor_ConnectionIsNull_ThrowsArgumentNullException()
        {
            ...
            // Act
            var ex = Assert.Throws<ArgumentNullException>(() => new NpgsqlCruderLab(null, "public"));
        ...
        public void Constructor_ConnectionIsNotOpen_ThrowsArgumentException()
        {
            ...
            // Act
            var ex = Assert.Throws<ArgumentException>(() => new NpgsqlCruderLab(connection, "public"));
        ...
        {
            // Arrange
            IDbCruder cruder = new NpgsqlCruderLab(this.Connection, "bad_schema");
        ...
        public void InsertRow_AllDataTypes_RunsOk()
        {
            ...
            Assert.That(loadedRow["TheTinyInt"], Is.EqualTo((byte)17));
            Assert.That(loadedRow["TheSmallInt"], Is.EqualTo((short)11));
            Assert.That(loadedRow["TheInt"], Is.EqualTo(44));
            Assert.That(loadedRow["TheBigInt"], Is.EqualTo(777L));

            Assert.That(loadedRow["TheDecimal"], Is.EqualTo(11.2m));
            Assert.That(loadedRow["TheNumeric"], Is.EqualTo(22.3m));

            Assert.That(loadedRow["TheSmallMoney"], Is.EqualTo(123.06m));
            Assert.That(loadedRow["TheMoney"], Is.EqualTo(60.77m));

            Assert.That(loadedRow["TheReal"], Is.EqualTo((float)15.99));
            Assert.That(loadedRow["TheFloat"], Is.EqualTo(7001.555));
            ...
            Assert.That(loadedRow["TheDateTime2"], Is.EqualTo(DateTime.Parse("2015-03-07T05:06:33.777")));

            var dateTime = (DateTime)loadedRow["TheDateTimeOffset"];
            DateTimeOffset dateTimeOffset = dateTime;
            Assert.That(dateTimeOffset,
                Is.EqualTo(DateTimeOffset.Parse("2011-11-12T10:10:10+03:00")));

            Assert.That(loadedRow["TheSmallDateTime"], Is.EqualTo(DateTime.Parse("1970-04-08T11:11:11")));
            Assert.That(loadedRow["TheTime"], Is.EqualTo(TimeSpan.Parse("03:03:03")));

            Assert.That(loadedRow["TheChar"], Does.StartWith("abc"));
            Assert.That(loadedRow["TheVarChar"], Is.EqualTo("Andrey Kovalenko"));
            Assert.That(loadedRow["TheVarCharMax"], Is.EqualTo("Rocky Marciano"));

            Assert.That(loadedRow["TheNChar"], Does.StartWith("АБВ"));
            Assert.That(loadedRow["TheNVarChar"], Is.EqualTo("Андрей Коваленко"));
            Assert.That(loadedRow["TheNVarCharMax"], Is.EqualTo("Роки Марчиано"));

            CollectionAssert.AreEqual(new byte[] { 0x10, 0x20, 0x33 }, ((byte[])loadedRow["TheBinary"]).Take(3));
            CollectionAssert.AreEqual(new byte[] { 0xff, 0xee, 0xbb }, (byte[])loadedRow["TheVarBinary"]);
            CollectionAssert.AreEqual(new byte[] { 0x80, 0x90, 0xa0 }, (byte[])loadedRow["TheVarBinaryMax"]);
        }
...
CREATE TABLE ""zeta"".""MyTab""(
    ...
    ""Name"" character varying(100) DEFAULT 'Polly')
";
...
"));
        ...
        public void GetRow_ValidArguments_ReturnsRow()
        {
            ...
            Assert.That(insertedRow["TheDateTimeOffset"],
                Is.EqualTo(DateTimeOffset.Parse("2011-11-12T10:10:10+03:00")));
            Assert.That(insertedRow["TheSmallDateTime"], Is.EqualTo(DateTime.Parse("1970-04-08T11:11:11")));
        }
        ...
        public void UpdateRow_ValidArguments_UpdatesRow()
        {
            ...
            for (var i = 0; i < loadedRows.Length; i++)
            {
                ...
                Assert.That(loadedRow["TheBinary"], Is.EqualTo(new byte[] { 1, }));
        ...
        public void UpdateRow_AllDataTypes_RunsOk()
        {
            ...
            var dateTime = (DateTime)loadedRow["TheDateTimeOffset"];
            DateTimeOffset dateTimeOffset = dateTime;
            Assert.That(dateTimeOffset, Is.EqualTo(DateTimeOffset.Parse("2005-05-05")));
            ...
            Assert.That(loadedRow["TheBinary"], Is.EqualTo(new byte[] { 2, }));
